        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          <div>
            <div className="flex items-center gap-2.5 mb-4">
              ...
              <span className="font-heading font-bold text-white text-lg">
                Coatings<span className="text-cta">Insurance</span>
              </span>
            </div>
            <p className="font-body text-sm text-white/70 leading-relaxed mb-4">
              Specialized insurance for coating contractors — GL, CPL, and workers comp. Licensed in all 50 states since 2005.
            </p>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {SERVICES.map((s) => (
                <li key={s.slug}>
                  <Link href={`/services/${s.slug}`} className="font-body text-sm text-white/70 hover:text-cta transition-colors">
                    {s.title}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-2">
              {NAV_LINKS.map((l) => (
                <li key={l.href}>
                  <Link href={l.href} className="font-body text-sm text-white/70 hover:text-cta transition-colors">
                    {l.label}
                  </Link>
                </li>
              ))}
              <li><Link href="/quote" className="font-body text-sm text-cta hover:text-cta-dark transition-colors">Get a Quote</Link></li>
            </ul>
          </div>

          <div>
            <h3 className="font-heading font-bold text-white mb-4">Ready to Get Insured?</h3>
            <p className="font-body text-sm text-white/70 mb-4">Same-day quotes for coating contractors. Call or fill out our quick quote form.</p>
            <Link href="/quote" className="block w-full bg-cta text-white text-center px-4 py-3 rounded-lg font-body font-bold text-sm hover:bg-cta-dark transition-colors mb-3">
              Get a Free Quote
            </Link>
